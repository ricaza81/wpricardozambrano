	...
	class ReduxFramework_slides {
		...
		public function render() {
			$defaults    = array(
				...
				'content_title'  => __( 'Slide', 'brixel' )
			);
			$this->field = wp_parse_args( $this->field, $defaults );
			echo '<div class="redux-slides-accordion" data-new-content-title="' . esc_attr( sprintf( __( 'New %s', 'brixel' ), $this->field[ 'content_title' ] ) ) . '">';
			$x           = 0;
			...
			if ( isset( $this->value ) && is_array( $this->value ) && !empty( $this->value ) ) {
				...
				foreach ( $slides as $slide ) {
					...
					echo '<img class="redux-slides-image" id="image_image_id_' . $x . '" src="' . $slide[ 'thumb' ] . '" alt="slides" target="_blank" rel="external" />';
					echo '</a>';
					echo '</div>';
					echo '<div class="redux_slides_add_remove">';
					echo '<span class="button media_upload_button" id="add_' . $x . '">' . __( 'Upload', 'brixel' ) . '</span>';
					$hide = '';
					if ( empty( $slide[ 'image' ] ) || '' == $slide[ 'image' ] ) {
						$hide = ' hide';
					}
					echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $slide[ 'attachment_id' ] . '">' . __( 'Remove', 'brixel' ) . '</span>';
					echo '</div>' . "\n";
					...
					$placeholder = ( isset( $this->field[ 'placeholder' ][ 'title' ] ) ) ? esc_attr( $this->field[ 'placeholder' ][ 'title' ] ) : __( 'Title', 'brixel' );
					...
					if ( $this->field[ 'show' ][ 'description' ] ) {
						$placeholder = ( isset( $this->field[ 'placeholder' ][ 'description' ] ) ) ? esc_attr( $this->field[ 'placeholder' ][ 'description' ] ) : __( 'Description', 'brixel' );
					...
					$placeholder = ( isset( $this->field[ 'placeholder' ][ 'btitle' ] ) ) ? esc_attr( $this->field[ 'placeholder' ][ 'btitle' ] ) : __( 'Button Title', 'brixel' );
					echo '<li><input type="' . $title_type . '" id="' . $this->field[ 'id' ] . '-btitle_' . $x . '" name="' . $this->field[ 'name' ] . '[' . $x . '][btitle]' . $this->field[ 'name_suffix' ] . '" value="' . esc_attr( $slide[ 'btitle' ] ) . '" placeholder="' . $placeholder . '" class="full-text slide-btitle" /></li>';
					$placeholder = ( isset( $this->field[ 'placeholder' ][ 'url' ] ) ) ? esc_attr( $this->field[ 'placeholder' ][ 'url' ] ) : __( 'URL', 'brixel' );
					...
					echo '<li><a href="javascript:void(0);" class="button deletion redux-slides-remove">' . __( 'Delete', 'brixel' ) . '</a></li>';
					echo '</ul></div></fieldset></div>';
					$x ++;
				}
			}
			if ( 0 == $x ) {
				echo '<div class="redux-slides-accordion-group"><fieldset class="redux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="redux-slides-header">' . esc_attr( sprintf( __( 'New %s', 'brixel' ), $this->field[ 'content_title' ] ) ) . '</span></h3><div>';
				$hide = ' hide';
				echo '<div class="screenshot' . $hide . '">';
				echo '<a class="of-uploaded-image" href="">';
				echo '<img class="redux-slides-image" id="image_image_id_' . $x . '" src="" alt="slides" target="_blank" rel="external" />';
				echo '</a>';
				echo '</div>';
				//Upload controls DIV
				...
				echo '<span class="button media_upload_button" id="add_' . $x . '">' . __( 'Upload', 'brixel' ) . '</span>';
				echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $this->parent->args[ 'opt_name' ] . '[' . $this->field[ 'id' ] . '][attachment_id]">' . __( 'Remove', 'brixel' ) . '</span>';
				echo '</div>' . "\n";
				echo '<ul id="' . $this->field[ 'id' ] . '-ul" class="redux-slides-list">';
				if ( $this->field[ 'show' ][ 'title' ] ) {
					$title_type = "text";
				...
				$placeholder = ( isset( $this->field[ 'placeholder' ][ 'btitle' ] ) ) ? esc_attr( $this->field[ 'placeholder' ][ 'btitle' ] ) : __( 'Button Title', 'brixel' );
				echo '<li><input type="' . $title_type . '" id="' . $this->field[ 'id' ] . '-btitle_' . $x . '" name="' . $this->field[ 'name' ] . '[' . $x . '][btitle]' . $this->field[ 'name_suffix' ] . '" value="" placeholder="' . $placeholder . '" class="full-text slide-btitle" /></li>';
				$placeholder = ( isset( $this->field[ 'placeholder' ][ 'url' ] ) ) ? esc_attr( $this->field[ 'placeholder' ][ 'url' ] ) : __( 'URL', 'brixel' );
				if ( $this->field[ 'show' ][ 'url' ] ) {
					$url_type = "text";
				} else {
			...
			echo '</div><a href="javascript:void(0);" class="button redux-slides-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field[ 'name_suffix' ] . '">' . sprintf( __( 'Add %s', 'brixel' ), $this->field[ 'content_title' ] ) . '</a><br/>';
		}
